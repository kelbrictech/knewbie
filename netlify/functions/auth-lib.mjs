...
const COOKIE = 'knewbie_session';

export function getUsers(){
  const raw = process.env.KNEWBIE_USERS_JSON;
...
export function signSession(payload){
  const secret=process.env.KNEWBIE_SESSION_SECRET;
...
export function readSession(event){
  ...
  const cookie=(event.headers.cookie||'').split(';').map(x=>x.trim()).find(x=>x.startsWith(COOKIE+'='));
